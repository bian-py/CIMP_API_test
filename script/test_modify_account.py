...
from api.api_modify_account import ApiModifyAccount
...
class TestModifyAccount:
    ...
    def test_modify_account(self):
        self.modify = ApiModifyAccount().api_modify_account(api.account_url, self.id, "王大明")
        assert self.modify.status_code == 200
        assert self.modify.json()['ret'] == 0
        ApiDeleteAccount().api_delete_account(api.account_url, self.id)